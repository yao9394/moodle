...
";
    ...
    public function test_view_group_override() {
        ...
        $s .= $assign->get_renderer()->render($summary);
        $this->assertContains('Tuesday, 28 May 2019, 7:31 AM', $s);

        $o = '';
        // Admin user should see a group override duedate.
        $this->setAdminUser();
        $summary = $assign->get_assign_grading_summary_renderable($group1->id);
        $o .= $assign->get_renderer()->render($summary);
        $this->assertContains('Friday, 20 September 2019, 10:37 PM', $o);
        $summary = $assign->get_assign_grading_summary_renderable($group2->id);
        $s .= $assign->get_renderer()->render($summary);
        $this->assertContains('Friday, 7 June 2019, 5:37 PM', $s);
    }
}
